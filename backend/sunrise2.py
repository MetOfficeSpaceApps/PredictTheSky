...
from math import cos,sin,acos,asin,tan, pi  , atan
...
def GMST(year, month, day, hour):
    ...
    omega= (125.04 - 0.052954 *D)%360.0
    L = (280.47 + 0.98565 * D)%360.0
    epsilon = (23.4393 - 0.0000004 *D)%360.0
    
    omega_radians= omega * pi / 180.0
    L_radians = L  * pi / 180.0
    epsilon_radians = epsilon  * pi / 180.0
    
    delta_psi = -0.000319 * sin (omega_radians) - 0.000024 * sin (2*L_radians)
    eqeq = delta_psi * cos(epsilon_radians) 
    GMST = GMST%24.0
    GAST = (GMST) + eqeq
    GAST = GAST % 24.0
...
def altitudeAndAzimuth(year, month, day, hour, lat, long, acension, declination):
    ...
    LHA_plus_radians = LHA_plus * pi / 180
    LHA_minus = (gast - alpha)*15.0 - lambda1
    LHA_minus_radians = LHA_minus * pi / 180
    
    altitude = asin(cos(LHA_plus_radians)* cos(delta_radians) * cos(phi_radians)+sin(delta_radians)*sin(phi_radians) )
    azimuth = atan ( -sin(LHA_plus_radians)/(tan(delta_radians)*cos(phi_radians) - sin(phi_radians) * cos(LHA_plus_radians)) )
...
if __name__ == "__main__":  
    ...
    hourFraction = timenow.hour + timenow.minute/60.0 + timenow.second/3600.0
    jd1 = JulianDay(timenow.year, timenow.month, timenow.day, hourFraction )    
    K=timenow.year
    M= timenow.month
    I = timenow.day
    UT = timenow.hour
    [gmst,gast] = GMST(timenow.year, timenow.month, timenow.day,hourFraction )
    acension =  10.42
    declination = 10.45
    [altitude, azimuth] = altitudeAndAzimuth(timenow.year, timenow.month, timenow.day, hourFraction, exeterLatitude, exeterLongitude, acension, declination)
    print 'Current time is ' + str(timenow)
    print 'Julian Day is ' + str(jd1)
    print 'GMST is ' + str(int(gmst)) + ':' + str((gmst%1.0)*60.0)
    print 'GAST is ' + str(gast) + ':' + str((gmst%1.0)*60.0)
    print '(acension, declination) of moon is (' + str(acension) + ',' + str(declination) +')'
    print '(altitude, azimuth) of moon is (' + str(altitude) + ',' + str(azimuth) +')'
